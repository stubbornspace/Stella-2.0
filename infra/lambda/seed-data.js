...
const patientPlans = [
  ...
  },
  {
    patientId: "p-1022",
    startOffset: 74,
    activities: ["letter-target", "letter-find", "eye-pong"],
    counts: [7, 6, 4],
  },
  {
    patientId: "p-1023",
    startOffset: 88,
    activities: ["letter-find", "motor-sequence-builder"],
    counts: [6, 5],
  },
  {
    patientId: "p-1024",
    startOffset: 69,
    activities: ["letter-target", "eye-pong", "inhibition-challenge"],
    counts: [6, 5, 4],
  },
  {
    patientId: "p-1025",
  ...
  },
  {
    patientId: "p-1028",
    startOffset: 45,
    activities: ["letter-find", "eye-pong", "motor-sequence-builder"],
    counts: [5, 4, 3],
  },
  {
    patientId: "p-1029",
    startOffset: 42,
    activities: ["letter-target", "letter-find"],
    counts: [4, 3],
  },
  {
    patientId: "p-1030",
    startOffset: 38,
    activities: ["eye-pong", "inhibition-challenge"],
    counts: [6, 5],
  },
  {
    patientId: "p-1031",
    startOffset: 33,
    activities: ["letter-target", "letter-find", "motor-sequence-builder"],
    counts: [8, 6, 5],
  },
  {
    patientId: "p-1032",
    startOffset: 30,
    activities: ["letter-find", "inhibition-challenge"],
    counts: [5, 4],
  },
  {
    patientId: "p-1033",
    startOffset: 26,
    activities: ["letter-target", "eye-pong"],
    counts: [4, 3],
  },
  {
    patientId: "p-1034",
...
function dateFor(startOffset, sessionIndex, activityIndex) {
...
function metricPoint(seed, sessionIndex, patientId, sessionCount) {
  const base = seed % 9;
...
function sessionStatus(sessionIndex, patientId) {
  return sessionIndex > 1 && (sessionIndex + patientId.length) % 11 === 0
    ? "ended-early"
    : "completed";
}

function baseSession(patientId, activity, sessionIndex, activityIndex, startOffset) {
  return {
  ...
  const audioMode =
    sessionIndex % 3 === 0
      ...
      : sessionIndex % 2 === 0
        ? "music"
        : "metronome";
  const patterns = ["horizontal", "vertical", "diagonal", "mixed"];
...
function makeInhibitionChallenge(
  ...
  const presets = ["balanced", "go-heavy", "stop-heavy"];
...
function makeMotorSequenceBuilder(
  ...
  const types = ["letter-sequence", "word-sequence"];
  const sequenceLength = 2 + (sessionIndex % 4);
  const audioModes = ["silent", "metronome", "music"];
...
const seedSessions = patientPlans.flatMap((plan) =>
